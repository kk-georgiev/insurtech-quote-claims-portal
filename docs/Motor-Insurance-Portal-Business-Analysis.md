...
**Проект:** InsurTech — Motor Insurance Quote & Claims Portal  
**Препоръчан екип:** 2–3 души  
**Предложен стек:** Java, Spring Boot, React, TypeScript, PostgreSQL  
**Сложност:** Висока
...
16. [План за реализация](#16-план-за-реализация)
17. [Разпределение на работата](#17-разпределение-на-работата)
18. [Стратегия за тестване](#18-стратегия-за-тестване)
19. [MVP и приоритети](#19-mvp-и-приоритети)
20. [Бъдещо развитие](#20-бъдещо-развитие)
21. [Рискове](#21-рискове)
22. [Отворени бизнес решения](#22-отворени-бизнес-решения)
23. [README и представяне](#23-readme-и-представяне)
24. [Заключение](#24-заключение)

---

## 1. Резюме и препоръка

Java и React са много подходящ избор за проекта. За екип от 2–3 души най-практичният подход е:
...
Проектът не трябва да бъде просто CRUD приложение. Най-важната му част са бизнес правилата: ценообразуване, права на потребителите, валидност на полицата и контролирани промени на статусите.
...
Добавянето на роля `ADMIN` е препоръчително, защото конфигурируемата тарифа трябва да има собственик. Ако екипът не иска четвърта роля, определени агенти могат да получат отделно разрешение `MANAGE_TARIFFS`.

Проверката на правата трябва да се прави в backend-а. Скриването на бутон в React не е достатъчна защита.
...
При едновременна работа от двама ликвидатори може да се използва optimistic locking чрез `@Version`.
...
За екип от 2–3 души microservices биха добавили значителна работа по комуникация, authentication, deployment и distributed transactions. Подходящото решение е един Spring Boot backend, разделен на ясни бизнес модули.
...
### 10.2. Препоръчан технологичен стек

| Част | Технологии |
|---|---|
| Backend | Java 21 LTS, Spring Boot, Maven |
| REST API | Spring Web, OpenAPI/Swagger |
...
| UI | Material UI или сходна библиотека |
| Backend tests | JUnit, Mockito, MockMvc, Testcontainers |
...
За server state е подходящ TanStack Query. Redux не е необходим, освен ако по-късно не се появи сложен глобален client state.
...
## 16. План за реализация

### Етап 1: Анализ и основа

- уточняване на роли и права;
- дефиниране на статуси и преходи;
- ER диаграма;
- monorepo структура;
- PostgreSQL и Flyway;
- authentication и authorization.

### Етап 2: Първи работещ вертикален процес

Възможно най-рано трябва да работи опростеният сценарий:

```text
Вход → Оферта → Полица → Щета → Одобрение → Известие
```

Първоначално тарифата и UI могат да бъдат базови. Целта е frontend и backend да бъдат интегрирани рано.

### Етап 3: Quote и тарифи
...
- snapshot;
- списък и детайли;
- проверка за повторно издаване.

### Етап 5: Щети

- FNOL форма;
- снимки;
- ликвидаторски интерфейс;
- status transitions;
- история;
- одобрени и изплатени суми.
...
- Docker Compose;
- Swagger;
- README и demo сценарий.

---

## 17. Разпределение на работата

### Екип от трима

| Човек | Основна отговорност |
|---|---|
| 1 | Pricing, quotes, policies и database модел |
| 2 | Claims, attachments, notifications и security |
| 3 | React UI, forms, dashboards и API интеграция |

Всички участват в code review, тестовете, документацията и API договорите.

### Екип от двама

- първият започва backend, database и бизнес логика;
- вторият започва frontend, форми и UX;
- след създаването на API основата работата продължава по вертикални функционалности;
- знанията не трябва да остават само при един човек.

Препоръчват се кратки feature branches и pull requests. Интеграцията не трябва да се оставя за последните дни.

---

## 18. Стратегия за тестване
...
## 22. Отворени бизнес решения

Следващите решения трябва да се потвърдят с ментор или Product Owner преди основната реализация:

| Въпрос | Препоръка за MVP |
|---|---|
| Клиентът и водачът едно лице ли са? | Да |
| Колко време важи офертата? | Например 14 дни, конфигурируемо |
| Как се активира полицата? | При приемане, без реално плащане |
| Какъв е периодът на покритието? | Начална дата + една година или избираем период |
| Какви са правата на агента? | Оферти и полици от името на клиент |
| Кой управлява тарифата? | Admin или отделно agent permission |
| Как се изпращат известия? | Само in-app за MVP |
| Кой маркира щетата като платена? | Ликвидатор или отделно разрешение |
| Колко снимки се позволяват? | Например 1–5 |
| Може ли щетата да се редактира? | Само преди начало на преглед |
| Има ли реално плащане? | Не, само регистриране на сума |
| Каква е валутата? | Изрично конфигурирано ISO поле |
| Може ли да има повече щети по една полица? | Да, ако са различни инциденти |
| Изисква ли се причина при отказ? | Да, задължително |

Това са предложени решения, защото условието не дава точен отговор на всички въпроси.

---

## 23. README и представяне
...
Подгответе един ясен demo сценарий и необходимите seed данни. Демонстрацията трябва да показва основния бизнес поток, а не само отделни екрани.

---

## 24. Заключение

Препоръчаната основа е:
...
Spring Boot modular monolith
        ↓
PostgreSQL + отделно file storage
```

Най-важните елементи за успешен проект са:

1. обясним и тестируем Quote Engine;
2. правилна връзка `Quote → Policy → Claim`;
3. ясни роли и server-side authorization;
4. контролирани status transitions;
5. история, audit и snapshots;
6. чиста модулна архитектура;
7. ранна frontend-backend интеграция;
8. убедителна демонстрация и документация.

Първо трябва да бъде завършен работещият end-to-end процес. След това могат да се добавят tariff versions, PDF полица, fraud flags и по-сложни интеграции.
