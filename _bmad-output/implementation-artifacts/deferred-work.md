...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-1-attachment-storage-and-validated-upload.md`
  ...
  status: RESOLVED 2026-09-02 (Story 10.2) — `GlobalExceptionHandler.handleMaxUploadSizeExceeded` added, mapping `MaxUploadSizeExceededException` to a 400 reusing the existing `ATTACHMENT_TOO_LARGE` code (a framework exception, not a module one, so no boundary is inverted). Covered by `ClaimControllerTest.fileOverSpringsOwnMultipartCap_isACleanBadRequestNotAnOpaqueServerError`, which posts an 11MB file against the real Spring servlet-level cap.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-1-attachment-storage-and-validated-upload.md`
  ...
  status: PARTIALLY RESOLVED 2026-09-02 (Story 10.2) — `AttachmentValidator.validateCount(int)` added; `ClaimController.submit` calls it against the raw `MultipartFile` list's size before any `getBytes()` call, so a batch over the count cap is rejected before any of its bytes are read. The no-aggregate-batch-cap observation (up to `max-count` x `max-file-size-bytes` still gets materialized once the count check passes) remains open — a genuinely separate, smaller limit, not this story's scope.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-10-1-attachment-storage-and-validated-upload.md`
  ...
  status: RESOLVED 2026-09-02 (Story 10.2) — `AttachmentValidator.displayName` now strips `\p{Cntrl}` characters and caps the result to 255 characters before it can reach an exception, a field error, or `StoredFile`. The cap matches `attachments.display_filename VARCHAR(255)` (`V11__create_attachments_table.sql`), the first column this value is persisted into.
